 ...
 * @returns 随机字符串
 */
export function generateString(length: number = 10, chars: string = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789'): string {
  let result = '';
  for (let i = 0; i < length; i++) {
    result += chars.charAt(Math.floor(Math.random() * chars.length));
  }
 ...
 * @returns 弹幕数据数组
 */
export function generateDanmaku(count: number = 10, overrides: Partial<DanmuData> = {}): DanmuData[] {
 ...
 * @returns 评论数据数组
 */
export function generateComments(count: number = 5, overrides: Partial<CommentData> = {}): CommentData[] {
 ...
 * @returns 配置数据数组
 */
export function generateConfigs(count: number = 5, overrides: Partial<ConfigData> = {}): ConfigData[] {
...
export class TestDataFactory {
   ...
   * @returns 生成的数据
   */
  static generate<T>(type: string, count: number = 1, overrides: any = {}): T | T[] {
    switch (type.toLowerCase()) {
      case 'video':
        return count === 1 ? generateVideo(overrides) as T : generateVideos(count, overrides) as T[];
      case 'danmu':
        return count === 1 ? generateDanmu(overrides) as T : generateDanmaku(count, overrides) as T[];
      case 'user':
        return count === 1 ? generateUser(overrides) as T : generateUsers(count, overrides) as T[];
      case 'comment':
        return count === 1 ? generateComment(overrides) as T : generateComments(count, overrides) as T[];
      case 'theme':
        return count === 1 ? generateTheme(overrides) as T : generateThemes(count, overrides) as T[];
      case 'config':
        return count === 1 ? generateConfig(overrides) as T : generateConfigs(count, overrides) as T[];
      default:
   ...
   * @returns 随机数据
   */
  static random(type: string, options: any = {}): any {
    switch (type.toLowerCase()) {
      case 'id':
        return generateId(options.prefix, options.length);
      case 'string':
        return generateString(options.length, options.chars);
      case 'number':
        return generateNumber(options.min, options.max);
      case 'boolean':
        return generateBoolean();
      case 'date':
        return generateDate(options.start, options.end);
      case 'email':
      ...
      case 'select':
        return randomSelect(options.array);
      case 'select-multiple':
        return randomSelectMultiple(options.array, options.count);
      default:
   ...
   * @returns 批量测试数据
   */
  static batch(definitions: Array<{
    type: string;
    key: string;
    count: number;
    overrides?: any;
    ...
    definitions.forEach(def => {
      result[def.key] = TestDataFactory.generate(def.type, def.count, def.overrides);
    });
