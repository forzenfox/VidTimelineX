...
describe("VideoModal组件测试", () => {
  ...
  /**
   * 测试用例 TC-004: 视频标签显示测试
   * 测试目标：验证视频标签正确显示
   */
  test("TC-004: 视频标签显示测试", () => {
    render(<VideoModal video={mockVideo} theme="blood" onClose={mockOnClose} />);
  ...
  /**
   * 测试用例 TC-005: 视频播放器iframe测试
   * 测试目标：验证视频播放器iframe正确渲染
   */
  test("TC-005: 视频播放器iframe测试", () => {
    render(<VideoModal video={mockVideo} theme="blood" onClose={mockOnClose} />);
  ...
  /**
   * 测试用例 TC-008: 背景点击关闭测试
   * 测试目标：验证点击背景遮罩触发onClose回调
   */
  test("TC-008: 背景点击关闭测试", () => {
    const { container } = render(
      <VideoModal video={mockVideo} theme="blood" onClose={mockOnClose} />
    );
  ...
  /**
   * 测试用例 TC-009: 模态框内容点击不关闭测试
   * 测试目标：验证点击模态框内容不触发onClose
   */
  test("TC-009: 模态框内容点击不关闭测试", () => {
    render(<VideoModal video={mockVideo} theme="blood" onClose={mockOnClose} />);
  ...
  /**
   * 测试用例 TC-010: 血怒模式样式测试
   * 测试目标：验证血怒模式下正确应用样式
   */
  test("TC-010: 血怒模式样式测试", () => {
    const { container } = render(
      <VideoModal video={mockVideo} theme="blood" onClose={mockOnClose} />
    );
    ...
    const modalContainer = container.querySelector(
      "div[style*='border: 2px solid rgba(225, 29, 72, 0.5)']"
    );
    expect(modalContainer).toBeInTheDocument();
  });

  /**
   * 测试用例 TC-011: 混躺模式样式测试
   * 测试目标：验证混躺模式下正确应用样式
   */
  test("TC-011: 混躺模式样式测试", () => {
    const { container } = render(
      <VideoModal video={mockVideo} theme="mix" onClose={mockOnClose} />
    );

    // 验证混躺主题的边框颜色
    const modalContainer = container.querySelector(
      "div[style*='border: 2px solid rgba(245, 158, 11, 0.5)']"
    );
    expect(modalContainer).toBeInTheDocument();
  });
  ...
  /**
   * 测试用例 TC-013: 高z-index测试
   * 测试目标：验证模态框具有高z-index确保在最上层
   */
  test("TC-013: 高z-index测试", () => {
    const { container } = render(
      <VideoModal video={mockVideo} theme="blood" onClose={mockOnClose} />
    );
