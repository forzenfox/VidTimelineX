...
import type { DanmakuTheme, ColorType } from './types';
...
describe('弹幕工具函数模块', () => {
  ...
  describe('getThemeColor 函数', () => {
    ...
    it('当主题不存在时应该返回 mix 主题的颜色', () => {
      expect(getThemeColor('invalid' as any, 'primary')).toBe('#9B59B6');
      expect(getThemeColor('unknown' as any, 'secondary')).toBe('#BE93D4');
    });
  ...
  describe('getDanmakuColor 函数', () => {
    ...
    it('当主题不存在时应该返回 mix 主题的随机颜色', () => {
      const validColors = ['#9B59B6', '#BE93D4', '#8E44AD'];
      const result = getDanmakuColor('invalid' as any);
      expect(validColors).toContain(result);
    });
