...
export function HorizontalDanmaku({ theme, isVisible = true }: HorizontalDanmakuProps) {
  ...
  const durationMap = useMemo(() => {
    ...
    // 获取弹幕池总数量
    const themeDanmaku = theme === "tiger" ? danmakuData.tigerDanmaku : danmakuData.sweetDanmaku;
  ...
  const danmakuList = useMemo(() => {
    ...
    for (let i = 0; i < danmakuCount && i < pool.length; i++) {
      ...
      // 根据主题设置颜色
      const color = theme === "tiger" ? "#E74C3C" : "#FF69B4";
  ...
  return (
    ...
    >
      {/* CSS 动画定义 */}
      <style>{`
        @keyframes tiantong-danmaku-scroll {
          0% {
      ...
      {danmakuList.map(item => (
        <div
          ...
          style={{
            ...
            transform: "translateX(100vw)",
            opacity: 1,
            animation: `tiantong-danmaku-scroll ${item.duration}s linear ${item.delay}s forwards`,
