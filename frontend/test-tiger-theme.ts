...
async function testTigerTheme() {
  ...
  try {
    ...
    const hasTigerStripe = await page.evaluate(() => {
      ...
      return body.classList.contains('tiger-stripe') || 
             body.closest('.tiger-stripe') !== null ||
             body.style.backgroundImage.includes('linear-gradient') ||
             document.querySelector('.tiger-stripe') !== null;
    });
    console.log('   虎纹背景:', hasTigerStripe ? '✅ 存在' : '❌ 不存在');
    
    // 3. 验证主题切换按钮是否正常工作
    console.log('\n3. 验证主题切换按钮...');
    const themeToggle = await page.locator('button[aria-label*="切换到"]');
    const isThemeToggleVisible = await themeToggle.isVisible();
    console.log('   主题切换按钮可见性:', isThemeToggleVisible ? '✅ 可见' : '❌ 不可见');
    
    // 4. 验证视频卡片是否有虎纹效果
    console.log('\n4. 验证视频卡片虎纹效果...');
    const videoCards = await page.locator('[role="article"]');
    const videoCardCount = await videoCards.count();
    console.log(`   视频卡片数量: ${videoCardCount}`);
    
    if (videoCardCount > 0) {
      const hasTigerStripeOverlay = await videoCards.first().evaluate(card => {
        return card.querySelector('.tiger-stripe-overlay') !== null;
      });
    ...
    // 5. 验证分类标签是否有虎纹背景
    console.log('\n5. 验证分类标签虎纹背景...');
    const categoryTags = await page.locator('[role="badge"]');
    const categoryTagCount = await categoryTags.count();
    console.log(`   分类标签数量: ${categoryTagCount}`);
    
    if (categoryTagCount > 0) {
      const hasTigerTagBg = await categoryTags.first().evaluate(tag => {
    ...
    // 6. 验证页面边角装饰图标
    console.log('\n6. 验证页面边角装饰图标...');
    const cornerIcons = await page.locator('div[class*="absolute"]:has-text("🐯")');
