...
import { dongzhuDanmaku, kaigeDanmaku } from '../data/danmaku';
...
export function SideDanmaku({ theme }: SideDanmakuProps) {
  ...
  useEffect(() => {
    // 选择弹幕池
    const pool = theme === 'dongzhu' ? dongzhuDanmaku : kaigeDanmaku;
    
    // 初始化消息列表
    const initialMessages: DanmakuMessage[] = Array.from({ length: 50 }, (_, i) => ({
      id: `msg-${Date.now()}-${i}`,
    ...
    setDisplayMessages(initialMessages.slice(-15)); // 显示最新15条
    ...
    }, 2000 + Math.random() * 2000); // 2-4秒添加一条
