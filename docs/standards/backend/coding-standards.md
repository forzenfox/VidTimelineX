...
#### 3.4.1 问题复盘：测试数据污染生产环境

**问题描述**：
在 `test_timeline_generator.py` 的 `test_run` 测试方法中，直接调用了 `self.generator.run(self.test_videos, "lvjiang")`，将测试数据写入了生产数据文件 `backend/data/lvjiang/videos.json`，导致测试数据（"测试视频1"、"测试视频2"）混入了生产环境。

**根本原因**：
1. 测试用例使用了生产数据类型 `"lvjiang"` 作为参数
2. `run` 方法通过 `get_data_paths("lvjiang")` 获取到生产文件路径
3. 测试数据被直接写入生产文件，而非隔离的测试环境

**影响范围**：
- 生产数据文件被污染，包含 2 条测试数据
- 前端展示时出现测试视频条目
- 数据完整性受损
...
测试用例必须确保测试数据不会写入生产数据文件。所有测试数据应写入专门的测试目录或临时文件。

**原则 2：使用专用测试数据类型**

在 `config.json` 中配置专门的 `test` 数据类型，用于测试目的：
...
```json
{
  "datatype": {
    ...
    "test": {
      "favorites_url": "",
      "backend_timeline_file": "data/test/videos.json",
      "frontend_timeline_file": "data/test/frontend_videos.json"
    }
  }
}
```
...
**原则 4：测试数据目录加入 .gitignore**

在 `.gitignore` 中添加测试数据目录，避免测试数据被提交到版本控制：
...
测试用例应在 `tearDown` 或测试结束后清理测试数据：

```python
def tearDown(self):
    """测试清理"""
    # 清理测试生成的文件
    test_file = Path("data/test/videos.json")
    if test_file.exists():
        test_file.unlink()
```
...
- [ ] 测试用例是否使用了 `"test"` 数据类型？
- [ ] 测试数据是否写入隔离的目录（`data/test/`）？
- [ ] 测试数据目录是否已添加到 `.gitignore`？
- [ ] 测试完成后是否清理了测试数据？
- [ ] 测试用例是否使用了临时文件或 mock 对象？
...
class TestDataProcessor(unittest.TestCase):
    ...
    def test_save_data(self):
        ...
        processor.save_to_file(self.test_data, self.test_file)
...
class TestTimelineGenerator(unittest.TestCase):
    @patch('src.crawler.timeline_generator.get_data_paths')
    def test_run_with_mock(self, mock_get_data_paths):
        """使用 mock 测试，避免写入真实文件"""
        # 配置 mock 返回临时路径
        mock_get_data_paths.return_value = {
            'TIMELINE_FILE': Path('/tmp/test_timeline.json')
        }
        
        generator = TimelineGenerator()
        result = generator.run(self.test_videos, "lvjiang")
