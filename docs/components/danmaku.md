...
> 本文档从 `frontend/src/shared/danmaku/README.md` 复制而来，用于集中管理组件文档。
...
```typescript
const { danmakuList, currentDanmaku, addDanmaku, isPlaying, setPlaying } = useDanmaku({
  defaultTheme: "blood",
  autoPlay: true,
  danmakuType: "sidebar",
});
...
```typescript
const { pool, addToPool, getNextAvailableTrack, occupyTrack, releaseTrack } = useDanmakuPool({
  maxCapacity: 100,
  displaySpeed: 1000,
  trackCount: 8,
  enableFilter: true,
...
const SidebarDanmakuExample: React.FC = () => {
  ...
  const { danmakuList, currentDanmaku, addDanmaku, clearDanmaku, isPlaying, setPlaying } =
    useDanmaku({
      defaultTheme: "mix",
      defaultSize: "medium",
      autoPlay: true,
  ...
  return (
    <div style={{ display: "flex", gap: "20px", padding: "20px" }}>
      ...
      >
        <h3 style={{ marginTop: 0, marginBottom: "16px" }}>侧边栏弹幕（{danmakuList.length}条）</h3>
        ...
        {currentDanmaku.length === 0 && (
          <div style={{ textAlign: "center", color: "#999", marginTop: "100px" }}>暂无弹幕</div>
        )}
      </div>
...
const HorizontalDanmakuExample: React.FC = () => {
  ...
  return (
    <div style={{ padding: "20px" }}>
      <h3 style={{ marginTop: 0, marginBottom: "16px" }}>飘屏弹幕示例</h3>
      ...
      <div style={{ marginBottom: "16px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px" }}>
          <span style={{ fontSize: "12px", width: "40px" }}>{formatTime(currentTime)}</span>
          ...
          <span style={{ fontSize: "12px", width: "40px" }}>{formatTime(videoDuration)}</span>
        </div>
...
const filterDanmaku = (messages: DanmakuMessage[]): DanmakuMessage[] => {
  return messages.filter(message => {
    ...
    // 过滤敏感词
    const hasSensitiveWord = SENSITIVE_WORDS.some(word => message.text.includes(word));
...
const preloadDanmaku = (currentTime: number) => {
  const futureDanmaku = getDanmakuByTimeRange(currentTime, currentTime + 5000);
...
describe("弹幕集成测试", () => {
  it("应该正确处理弹幕的添加和移除", () => {
    const { result } = renderHook(() => useDanmaku({ defaultTheme: "mix" }));
...
**源文件位置**: `frontend/src/shared/danmaku/README.md`
