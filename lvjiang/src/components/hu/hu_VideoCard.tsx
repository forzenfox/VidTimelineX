    ...
    >
      ...
      <div className="relative aspect-video overflow-hidden">
        ...
        <div className="absolute inset-0 bg-black/20 group-hover:bg-black/30 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100">
          <div className="w-14 h-14 bg-white/95 rounded-full flex items-center justify-center shadow-lg transform scale-50 group-hover:scale-100 transition-transform duration-300" aria-hidden="true">
            <Play fill="var(--primary)" className="text-primary ml-1" size={28} />
          </div>
        </div>
        
        {/* Category Tag - 分类角标优化 */}
        <div className="absolute top-2.5 left-2.5 px-2.5 py-1 bg-primary/90 backdrop-blur-sm text-primary-foreground text-xs font-bold rounded-lg flex items-center shadow-md" role="badge">
      ...
      <div className="p-4 sm:p-4.5">
        <h3 id={`video-title-${video.id}`} className="font-bold text-foreground line-clamp-2 min-h-[2.75rem] leading-[1.5] group-hover:text-primary transition-colors text-sm sm:text-base">
          {video.title}
        </h3>
        
        {/* 日期与播放量放在同一行 */}
        <div className="flex items-center justify-between mt-3 text-xs sm:text-sm text-muted-foreground" aria-label="视频信息">
